...
generator = MarketDataGenerator(0.8, seq_length, output_count, batch_size)
predictor = DeepPredictor(generator, encoder_layers, decoder_layers, isAttending=True)
...
plt.show()
plt.plot(testY_reshaped[:, 1, 0])
plt.plot(testPredict_reshaped[:,1,0])
plt.show()
plt.plot(testY_reshaped[:, 2, 0])
plt.plot(testPredict_reshaped[:,2,0])
plt.show()
